...
| 6 | **Scheduler trigger / Sync Now decision** | ADR + decision record | 2-5 landed (or explicitly descoped by ADR) | decision recorded: weekly job vs on-demand + Sync Now endpoint (FR-27/FR-29); if implemented: run lock + job + endpoint with tests (resolves BLOCKER-SCHED-01) |
