...
| Knowledge desk (Notes) | Reference knowledge + links | Nav KNOWLEDGE → Knowledge (`nav-knowledge`) | New note / toolbar actions / Summarize·Extract tasks with AI (editor) ✅ P17-029 | Desk toolbar explainer (§31) | empty: education / success: Saving…→Saved ✓ / failure: save flash ⚪ pending token pass | note edits queue ⚪ | notes linked to entities enrich context panels ⚪; AI proposals stay pending until accepted (FR-62) | Links, Attachments, Canvas embeds | ui-audit CTA ✅, accessibility ✅, journey K ✅ P17-029, ai-action-audit ✅ P17-025 |
| Canvas | Free-form visual thinking | Nav KNOWLEDGE → Canvas (`nav-canvas`) | Excalidraw-owned tools / Kinevo shell chrome | External boundary notice (§104 rule) | empty: blank scene + Suggest structure with AI (boards index) ✅ P17-029 / success: scene autosaves ⚪ / failure: Excalidraw-owned | canvas ops queue ⚪ | none (deliberate boundary — AI proposes sections, Kinevo persists) | Notes (embeds), exports | canvas-hardening ✅, keyboard flow ✅, journey K ✅ P17-029, ai-action-audit gate ✅ P17-025 |
| Analytics | Read the truth; choose next adaptation | Nav REVIEW → Analytics (`nav-analytics`) | — (read-only) / range·pillar filters, per-section actions | Executive-signal-first hierarchy; ChartMeta on every chart | empty: education block / success: signal → interpretation → action / failure: per-section error rows | read-only; safe on flaky network | IS the analytics surface: overload, imbalance, completion, goal pressure → routed actions | Every producer surface; routes into Goals/Today/Schedule | analytics-hierarchy ✅, journey-j ✅, canonical-journey ✅ P17-023 |
