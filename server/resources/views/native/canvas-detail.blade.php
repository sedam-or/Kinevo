<native:top-bar title="Canvas" background-color="#0a0a0a" text-color="#EDEDEC" />
<native:scroll-view class="bg-theme-bg">
    <native:column class="p-4 gap-3">
        ...
        @elseif ($screen->state === 'loading')
            <native:text class="text-theme-muted">Loading canvas…</native:text>
        @elseif ($screen->state === 'conflict')
            <native:column class="p-4 gap-2 rounded-md border-theme-warning border-2 bg-theme-surface">
                <native:row class="gap-2 items-center">
                    <native:icon name="sync_problem" color="#8A5A00" size="20" />
                ...
                <native:pressable ref="cd-reload-conflict" a11y-label="Reload canvas" class="p-4 rounded-sm bg-theme-primary border-theme-border border-2" @tap="reload">
        ...
        @elseif ($screen->state === 'offline' || $screen->state === 'error')
            <native:column class="p-4 gap-2 rounded-md border-theme-danger border-2 bg-theme-surface">
                ...
                <native:pressable ref="cd-retry" a11y-label="Retry loading canvas" class="p-4 rounded-sm bg-theme-primary border-theme-border border-2" @tap="reload">
        ...
        @else
            ...
            <native:column class="p-4 gap-2 rounded-md border-theme-border border-4 bg-theme-surface">
                <native:row class="gap-2 items-center">
                    <native:icon name="draw" color="#DE3005" size="24" />
                    <native:text class="text-lg font-bold text-theme-text">{{ $screen->canvas['title'] ?? 'Untitled' }}</native:text>
                </native:row>
                <native:text class="text-sm text-theme-muted">Canvas #{{ $screen->canvasId }}</native:text>
            </native:column>

            <native:column class="p-4 gap-2 rounded-md border-theme-border border-2 bg-theme-surface">
                <native:text class="text-sm font-bold text-theme-muted">SYNC STATUS</native:text>
                <native:text class="text-sm text-theme-muted">Read-only mirror. Excalidraw owns drawing on the web app.</native:text>
            </native:column>

            <native:text class="text-sm font-bold text-theme-muted">LINKED ENTITIES ({{ count($screen->links) }})</native:text>
            @forelse ($screen->links as $link)
                <native:row class="p-4 gap-2 items-center justify-between rounded-md border-theme-border border-2 bg-theme-surface" :key="$link['id'] ?? $loop->index">
                    ...
                    <native:pressable ref="cd-unlink-{{ $link['id'] ?? $loop->index }}" a11y-label="Remove link" class="p-2 rounded-sm bg-theme-surface border-theme-border border-2" @tap="removeLink({{ $link['id'] ?? 0 }})">
            ...
            @empty
                <native:column class="p-3 rounded-md border-theme-border border-2 bg-theme-surface">
                    <native:text class="text-theme-muted">No linked entities yet.</native:text>
                </native:column>
            @endforelse

            <native:text class="text-sm font-bold text-theme-muted">QUICK LINK A TASK</native:text>
            <native:column class="p-2 gap-1 rounded-md border-theme-border border-2 bg-theme-surface">
                @forelse ($screen->tasks as $task)
                    <native:pressable ref="cd-link-task-{{ $task['id'] ?? $loop->index }}" a11y-label="Link to task {{ $task['title'] ?? 'Untitled' }}" class="p-4 rounded-sm bg-transparent" @tap="linkTask({{ $task['id'] ?? 0 }})">
            ...
            <native:pressable ref="cd-back" a11y-label="Back to canvases" class="p-4 rounded-sm bg-theme-primary border-theme-border border-2" @tap="backToCanvases">
                <native:text class="text-theme-on-primary font-bold">Back to Canvases</native:text>
            </native:pressable>
        @endif
    </native:column>
</native:scroll-view>
