...
/** Timeline rail node treatment per milestone state (tokens only). */
function milestoneNodeClass(status: string): string {
    if (status === 'completed') {
        return 'bg-success border-success';
    }
    if (status === 'active') {
        return 'bg-bg border-border ring-[3px] ring-focus';
    }
    if (status === 'dropped' || status === 'blocked') {
        return 'bg-danger border-danger';
    }
    return 'bg-bg border-border/60';
}
</script>

<template>
    <div class="flex flex-col gap-8" data-testid="goal-detail">
        <header class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
                <KButton variant="ghost" type="button" data-testid="goal-detail-back" @click="emit('back')"><KIcon name="arrow-left" :size="16" /> Back</KButton>
                <h1 class="text-xl font-semibold truncate" data-testid="goal-detail-title">{{ goals.currentGoal?.title ?? 'Goal' }}</h1>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                <span class="rounded-sm bg-surface border border-border px-2 py-0.5 text-xs" data-testid="goal-detail-status">
                    {{ goals.currentGoal ? statusLabel(goals.currentGoal.status) : '' }}
                </span>
                <!-- Page primary remains Accept/state actions; AI entry is secondary -->
                <KButton
                ...
                >{{ generating ? generationStage : 'Break Down with AI' }}</KButton>
            </div>
        </header>
        <NextActionBanner
            v-if="nextAction"
        ...
        <div v-if="goals.loading" class="text-sm text-text-muted" data-testid="goal-detail-loading">Loading…</div>
        ...
        <template v-if="goals.currentGoal">
            ...
            <!-- Deadline / Progress -->
            <section class="surface-metadata" data-testid="goal-meta">
                <div class="flex flex-wrap gap-2 text-sm">
                    <span class="rounded-sm bg-surface border border-border px-2 py-1">Horizon: {{ goals.currentGoal.horizon }}</span>
                    <span v-if="goals.currentGoal.target_date" class="rounded-sm bg-surface border border-border px-2 py-1">Deadline: {{ goals.currentGoal.target_date }}</span>
                </div>
                <!-- One dominant progress visualization (design.md §17, §39:
                     milestone roadmap preferred over multiple rings). -->
                <div class="mt-3" data-testid="goal-progress-bar" role="img" :aria-label="`${goals.currentGoal.progress}% complete`">
            ...
            <section class="surface-primary p-5" data-testid="goal-milestones">
                ...
                <div
                    ...
                    class="mb-4 flex flex-col items-center justify-center gap-3 border-2 border-dashed border-border/40 rounded-sm p-6 text-center"
                    data-testid="milestones-empty"
                >
                    <span class="text-sm text-text-muted">No milestones yet.</span>
                    <KButton
                        variant="secondary"
                ...
                <form class="flex flex-wrap gap-2 mb-4 items-end" @submit.prevent="addMilestone">
                    <input
                        ref="milestoneTitleInput"
                        v-model="milestoneForm.title"
                        type="text"
                        placeholder="Add milestone"
                        class="border border-border rounded-sm px-3 py-2 text-sm bg-bg text-text focus:outline-none focus-visible:ring-2 focus-visible:ring-focus flex-1 min-w-40"
                        data-testid="milestone-title"
                    />
                    <KInput v-model="milestoneForm.targetDate" type="date" data-testid="milestone-date" />
                    <KButton type="submit" variant="secondary" data-testid="milestone-add">Add</KButton>
                </form>

                <ol class="relative ml-2 space-y-5 border-l border-border/30" data-testid="milestone-timeline">
                    <li v-for="ms in sortedMilestones()" :key="ms.id" class="ml-4 relative" data-testid="milestone-item">
                        <span aria-hidden="true" class="absolute -left-[23px] top-1 h-3 w-3 rounded-full border" :class="milestoneNodeClass(ms.status)"></span>
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span class="shrink-0 text-xs rounded-sm bg-surface border border-border px-2 py-0.5">{{ statusLabel(ms.status) }}</span>
                        </div>
                        ...
                        <div class="flex gap-1 mt-1 flex-wrap">
                            <KButton
                                ...
                                :key="next"
                                variant="ghost"
                                class="!min-h-0 !px-2 !py-0.5 !text-xs"
                    ...
                    <li v-if="goals.milestones.length === 0" class="ml-4 text-sm text-text-muted">No milestones yet.</li>
                </ol>
            </section>
        </template>
    </div>
