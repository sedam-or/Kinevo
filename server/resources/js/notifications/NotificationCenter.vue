...
<template>
    <div class="relative" data-testid="notifications">
        <button
            type="button"
            class="flex items-center gap-1 text-sm border border-border rounded-sm px-2 py-1 transition-colors hover:bg-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-focus"
            :aria-expanded="open"
            aria-haspopup="true"
        ...
        <div
            v-if="open"
            class="absolute right-0 top-full z-[var(--z-popover)] mt-1 w-80 max-w-[90vw] rounded-sm border border-border bg-bg text-text shadow-rest p-3"
            data-testid="notification-center"
        >
            <div class="flex items-center justify-between mb-2">
                <strong class="font-mono text-[11px] uppercase tracking-widest text-text-muted">Notifications</strong>
                <button type="button" class="text-xs underline text-text-muted hover:text-text" data-testid="notifications-close" @click="open = false">close</button>
            </div>

            <div v-if="store.error" class="text-sm text-danger" role="alert" data-testid="notifications-error">{{ store.error }}</div>
            <div v-if="store.loading && store.notifications.length === 0" class="text-sm text-text-muted" data-testid="notifications-loading">Loading…</div>
            ...
            <template v-if="!store.loading || store.notifications.length > 0">
                <section v-for="group in (['unread', 'today', 'earlier'] as const)" :key="group" class="mb-3">
                    <div v-if="store.grouped[group].length > 0" class="font-mono text-[11px] uppercase tracking-widest text-text-muted mb-1">{{ group }}</div>
                    <ul v-if="store.grouped[group].length > 0" class="space-y-1">
                        <li
                            v-for="n in store.grouped[group]"
                            :key="n.id"
                            class="flex items-start gap-2 border border-border/20 rounded-sm px-2 py-2"
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="text-xs text-text-muted">{{ timestampFor(n) }}</div>
                            </div>
                ...
                <div v-if="store.notifications.length === 0 && !store.loading" class="text-sm text-text-muted" data-testid="notifications-empty">
