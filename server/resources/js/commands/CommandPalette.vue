...
<template>
    <div v-if="props.open" class="fixed inset-0 z-[var(--z-critical-overlay)] flex items-start justify-center bg-black/40 pt-24 px-4" @click.self="close()">
        <div class="w-full max-w-lg rounded-sm border border-gray-300 dark:border-gray-600 bg-surface shadow-rest p-3 flex flex-col gap-2" role="dialog" aria-modal="true" aria-label="Command palette" data-testid="command-palette">
            <input
                v-model="query"
                type="text"
                autofocus
            ...
            <ul class="flex flex-col gap-1 max-h-72 overflow-y-auto">
                <li v-for="command in filteredCommands" :key="command.id">
                    <button
                        type="button"
                        class="w-full flex items-center justify-between rounded-sm px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800 min-h-[44px]"
                        :data-testid="`command-${command.id}`"
                        @click="execute(command)"
                    >
                        <span>{{ command.label }}</span>
                        <span v-if="command.hint" class="text-xs text-gray-500 dark:text-gray-400">{{ command.hint }}</span>
                    </button>
                </li>
                <li v-if="filteredCommands.length === 0 && knowledgeResults.length === 0 && !searching" class="px-3 py-2 text-xs text-gray-500 dark:text-gray-400" data-testid="command-empty">
                    No matches. Type two characters or more to search notes.
                </li>
            </ul>

            <div v-if="knowledgeResults.length > 0" class="border-t border-gray-200 dark:border-gray-700 pt-2">
                <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1">Notes</p>
                <button
                    v-for="note in knowledgeResults"
                    :key="note.id"
                    type="button"
                    class="w-full text-left rounded-sm px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800 min-h-[44px]"
