...
namespace App\Domain\Recharge\ValueObjects;

use InvalidArgumentException;

/**
 * Recharge timer status (FR-05). Persisted state machine; transitions are
 * explicit and validated. Mirrors the execution timer state machine.
 */
final class RechargeStatus
{
    ...
    public function __construct(
        ...
        if (! in_array($value, self::STATUSES, true)) {
            throw new InvalidArgumentException("Unsupported recharge status: {$value}");
        }
    }
